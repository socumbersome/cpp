...
public:
   size_t table [ dimension ][ dimension ];
      ...
      // table[i][j] is ith row and jth column
      ...
      // Position where val must be in the solution.
   {
		...
		if(val < 0 || val >= dimension * dimension)
			throw std::runtime_error("that val dont appear in sol");
		if(val % dimension == 0)
		{
			return std::make_pair(val / dimension - 1, dimension - 1);
		}
		else
		{
			return std::make_pair(val / dimension, val % dimension - 1);
		}
   }  
