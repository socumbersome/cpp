...
template< typename M >
polynomial<M> operator + ( const polynomial<M> & pol1, 
	const polynomial<M> & pol2 )
{
   auto res = pol1;
   res += pol2;
   return res;
	...
	const polynomial<N> & pol2 )
{
   ...
   for( auto& p1 : pol1 )
   {
	   for(auto& p2 : pol2 )
	   {
		   varchain nch = p1.first * p2.first;
		   res.repr[nch] += p1.second * p2.second;
	   }
   }
   return res;
}
